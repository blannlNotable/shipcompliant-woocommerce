...
class Admin {
    ...
    public static function handle_post(): void {
        ...
        if (isset($_POST['scw_save_settings']) && check_admin_referer('scw_save_settings_nonce')) {
            ...
            // NEW: SalesOrder save/persist endpoint (required so commit can find SalesOrderKey)
            $s['endpoint_sales_order'] = sanitize_text_field($_POST['endpoint_sales_order'] ?? ($s['endpoint_sales_order'] ?? '/api/v1/salesOrders'));
        ...
        if (isset($_POST['scw_test']) && check_admin_referer('scw_test_nonce')) {
            ...
            if ($client->can_auth()) {
                ...
                $payload = [
                    'address' => [
                        'street1' => '1 Main St',
                        'city'    => 'Napa',   // FIXED: lowercase
                        'state'   => 'CA',
                        'zip1'    => '94558',  // FIXED: lowercase
                        'zip2'    => '',
                        'country' => 'US'
                    ]
                ];
            ...
            <form method="post">
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td><input class="regular-text" type="text" name="endpoint_validate_address" value="<?php echo esc_attr($s['endpoint_validate_address']); ?>"></td>
                    </tr>

                    <tr>
                        <th>SalesOrder (Save)</th>
                        <td><input class="regular-text" type="text" name="endpoint_sales_order" value="<?php echo esc_attr($s['endpoint_sales_order'] ?? '/api/v1/salesOrders'); ?>"></td>
                    </tr>

                    <tr>
                        <th>Quote</th>
                        <td><input class="regular-text" type="text" name="endpoint_quote" value="<?php echo esc_attr($s['endpoint_quote']); ?>"></td>
                    </tr>
                    <tr>
                        <th>Commit</th>
                        <td><input class="regular-text" type="text" name="endpoint_commit" value="<?php echo esc_attr($s['endpoint_commit']); ?>"></td>
                    </tr>
                    <tr>
                        <th>Tracking</th>
                        <td><input class="regular-text" type="text" name="endpoint_tracking" value="<?php echo esc_attr($s['endpoint_tracking']); ?>"></td>
                    </tr>
                </table>
